import React from "react";
import { Link } from "react-router-dom";
import { AiOutlineDownCircle } from "react-icons/ai";
import { IconContext } from "react-icons";

const Welcome = () => {
  return (
    <section className="text-white">
      <div className="row p-3 align-items-end" style={{ height: "460px" }}>
        <div className="col-6 col-lg-5 mb-5 offset-3 text-center">
          <h3 className="display-2 fst-italic">Intercessors Church</h3>
          <p>
            Lorem ipsum dolor sit amet consectetur. Eu donec feugiat sodales
            adipiscing netus
          </p>
          <a href="#experience" className="btn btn-outline-light rounded-5">
            Worship with us
          </a>
        </div>
        <div className="col-3 mb-5">
          <IconContext.Provider value={{ size: "30px" }}>
            <a href="#media" className="text-light nav-link float-end me-2">
              <AiOutlineDownCircle />
            </a>
          </IconContext.Provider>
        </div>
      </div>
    </section>
  );
};

export default Welcome;
